...
export function transformDateFormat() {
  const month = date.toLocaleString('en-US', { month: '2-digit', timeZone: 'America/Denver' });
  const day = date.toLocaleString('en-US', { day: '2-digit', timeZone: 'America/Denver' });
  const year = date.toLocaleString('en-US', { year: 'numeric', timeZone: 'America/Denver' });
  ...
  const newFormatDate = year.concat('-', month, '-', day, ' ', time);
...
export function getMountainTime() {
